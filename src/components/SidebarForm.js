    ...
    <form onSubmit={handleSubmit} className="border-r-2 pt-6 border-b-2 border-indigo-300">
        <div className=" grid grid-cols-2 grid-rows-4 items-center justify-items-center">
            ...
            <div className="flex justify-center align-center col-span-2 text-2xl p-4">
                ...
                <span className="pl-2">
                    <input className="border border-2 text-center rounded border-slate-300" onChange={(event)=>{setName(event.target.value)}} type="text" value={name} />
                </span>
            </div>
            <div className="flex-col items-start ">
                <label className="text-md pr-2">Number of Questions:</label>
                <input className="w-12 h-8 border border-2 text-center rounded border-slate-300" onChange={(event)=>{setQuestionAmount(event.target.value)}} type="number" value={questionAmount}  min="1" max="10" />
            </div>
            <div>
            ...
            <div className="col-span-2 flex items-center justify-center text-xl">
                ...
                <input className="border border-2 text-center rounded border-slate-300" type="date" onChange={(event) => {setDate(event.target.value)}} value={date} />
                <div className="pl-6 flex">
                    <input className="w-12 border border-2 text-center rounded border-slate-300" onChange={(event)=> {setTimeHour(event.target.value)}} value={timeHour} type="number" max="12" min="1" />
                    <label className="px-2 text-3xl">:</label>
                    <input className="w-12 border border-2 text-center rounded border-slate-300" onChange={(event)=> {setTimeMinute(event.target.value)}} value={timeMinute} type="number" max="59" min="00" />
